...
# On macOS the port looks like /dev/cu.usbmodemXXXX (check with: ls /dev/cu.usbmodem*)
arduino = serial.Serial('/dev/cu.usbserial-130', 9600) #Comment this line out to test out the program without an Arduino connected
...
# Smoothed angle states (initialized to the Arduino's startup pose)
...
while True:
	...
	if(math.isnan(arm2_angle)==False and math.isnan(arm1_angle)==False):
		...
		# Exponential smoothing (EMA) to remove frame-to-frame jitter
		SMOOTH = 0.4  # 0 = no movement, 1 = raw/no smoothing
		...
		# Send only when something actually changed (reduces serial traffic & jitter)
		if (s1, s2, st) != last_sent:
			print("Arm1 Angle is: {}  |  Arm2 Angle is: {}  |  Tilt Angle is: {}".format(s1, s2, st))
			# Sync byte (0xAA) + three angle bytes — must match the Arduino sketch
			arduino.write(struct.pack('>BBBB', 0xAA, s1, s2, st))
			last_sent = (s1, s2, st)
